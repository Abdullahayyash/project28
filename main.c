...
volatile int BPM = 0; 
volatile int Signal; 
volatile int IBI = 0; // Inter-Beat Interval
...
const char *patient_name = "Patient:Mustafa"; // Patient name

#define MAX_BPM_RECORDS 30
...
void calculate_heart_beat(int adc_value) {
    ...
    if (N > 250) { 
        if ((Signal > thresh) && (Pulse == false) && (N > 150)) {
            ...
            IBI = runningTotal; // Store the IBI
